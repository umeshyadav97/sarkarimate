...
type PreparationListItem = (typeof syllabusResponse.data.items)[number];
...
const preparationGuideOverrides: Record<
  string,
  {
    ...
    overviewHighlights?: PreparationMetric[];
...
> = {
  'ssc-cgl': {
    ...
    quickActions: [
      ...
      { label: 'View Latest Notification', href: '/job-details/ssc-cgl', type: 'notification' },
    ...
    resources: [
      ...
      },
      {
        ...
        description: 'Month-wise study plan for better preparation',
        href: '#study-plan',
        tone: 'blue',
      },
    ],
    ...
    faqs: [
      ...
      },
      {
        ...
        answer:
          'Coaching is optional. A clear syllabus, regular practice, previous year papers, and mock tests can also build strong preparation.',
      },
      {
...
export function getPreparationStaticParams() {
  return syllabusResponse.data.items.map((item) => ({ slug: item.slug }));
}

export function getPreparationSitemapEntries() {
...
export function getPreparationGuide(slug: string): PreparationGuide | null {
  ...
  return mapListItemToPreparationGuide(item);
}

function mapListItemToPreparationGuide(item: PreparationListItem): PreparationGuide {
  const template = categoryTemplates[item.category] ?? defaultTemplate;
  const override = preparationGuideOverrides[item.slug];
  const title = `${item.examName} Syllabus & Preparation Guide 2026`;
  const canonical = `/preparation/${item.slug}`;
  ...
  return {
    ...
    seoContent: [
      {
        title: `${item.examName} Preparation Guide`,
        body: [
          `${item.examName} preparation should begin with the latest syllabus, exam pattern, previous year papers, and a realistic study plan.`,
...
function createHeroFacts(item: PreparationListItem): PreparationMetric[] {
  return [
    { label: 'Conducting Body', value: item.organization, tone: 'blue' },
    { label: 'Exam Category', value: item.category, tone: 'blue' },
    { label: 'Guide Type', value: 'Syllabus & Preparation', tone: 'slate' },
    { label: 'Last Updated', value: formatDisplayDate(item.updatedDate), tone: 'orange' },
  ];
}

function createOverviewHighlights(item: PreparationListItem): PreparationMetric[] {
  return [
...
function createQuickActions(item: PreparationListItem) {
...
function getOptionalString(item: PreparationListItem, key: string) {
  const value = (item as Record<string, unknown>)[key];
  return typeof value === 'string' && value.length > 0 ? value : null;
}

function createExamPattern(template: CategoryTemplate): PreparationTable {
  return {
    columns: [
...
function createResources(item: PreparationListItem): PreparationResource[] {
  return [
    {
      title: 'Preparation Strategy',
      description: `Plan your ${item.examName} preparation with syllabus-first study.`,
      href: '#study-plan',
      tone: 'blue',
    },
    {
    ...
    {
      title: 'Study Plan',
      description: 'Follow a phased preparation roadmap.',
      href: '#study-plan',
      tone: 'slate',
    },
    {
...
function createCutoffTable(): PreparationTable {
  return {
    columns: [
      { key: 'year', label: 'Year' },
      { key: 'general', label: 'General' },
