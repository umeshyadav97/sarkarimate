...
export function SyllabusDataTable({ table }: SyllabusDataTableProps) {
            ...
            {table.rows.map((row) => (
              <tr key={row.id} className="hover:bg-blue-50/30">
                {table.columns.map((column) => (
                  <td
                    key={column.key}
                    className="border-r border-slate-100 px-4 py-3 text-center text-sm font-semibold leading-6 text-slate-800 last:border-r-0 first:text-left"
                  >
...
function TableValue({
  ...
  columnKey: string;
  value: SyllabusTable['rows'][number]['values'][string];
}) {
  if (!value) {
    return '-';
  }
